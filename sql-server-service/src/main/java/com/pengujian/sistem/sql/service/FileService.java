    ...
    public List<Boolean> prosesDanSaveData(MultipartFile file) throws Exception{
        ...
        try{
            if(file.getOriginalFilename().contains("ProcessLogLine")){
                ...
                results = service.saveProsesLogTable(logTables);
            }
        } catch (Exception e){
            e.printStackTrace();
            throw e;
